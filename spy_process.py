...
def process_file(file):
    ...
    '''
    dirname, filename = os.path.split(file)
    log(module=__name__, server='process', function='process_file', level='SELECT', dlgid='PROC00', msg='file={}'.format(filename))
    dlgid, *res = re.split('_', filename)
    log(module=__name__, server='process', function='process_file', level='SELECT', dlgid='PROC00', msg='DLG={}'.format(dlgid))
    bd = BD(modo=Config['MODO']['modo'], dlgid=dlgid, server='process')
    if bd.datasource == 'DS_ERROR':
        log(module=__name__, server='process', function='process_file', level='SELECT', dlgid='PROC00',msg='ERROR: DS not found !!')
    ...
    with open(file) as myfile:
        ...
        for bulkline in myfile:
            ...
            for line in lines_list:
                ...
                if 'DATE' in line:
                    #log(module=__name__, server='process', function='process_file', level='SELECT', dlgid='PROC00', msg='line={}'.format(line))
...
if __name__ == '__main__':
    ...
    log(module=__name__, server='process', function='main', console=console, dlgid='PROC00', msg='SERVER: dirname={}'.format(dirname))
    pid_list = list()
    #
    ...
    if len(sys.argv) > 1 and sys.argv[1] == 'DEBUG':
        ...
        while True:
            ...
            for file in file_list:
                log(module=__name__, server='process', function='main', level='SELECT', dlgid='PROC00', console=console,  msg='File {}'.format(file))
                process_file(file)
        ...
        for file in file_list:
            ...
            if len(pid_list) < MAXPROCESS:
                ...
                if pid == 0:
                    ...
                    pid_list.append(pid)
                    log(module=__name__, server='process', function='main', dlgid='PROC00', msg='SERVER: append child {}'.format(pid))
                    print('Server: append child {}'.format(pid))

            # No queda espacio: Espero que se vaya haciendo lugar
            while len(pid_list) == MAXPROCESS:
                log(module=__name__, server='process', function='main', dlgid='PROC00', msg='SERVER: List FULL: {}'.format(pid_list))
                ...
                for pid in pid_list:
                    if not psutil.pid_exists(pid):
                        pid_list.remove(pid)
                        log(module=__name__, server='process', function='main', dlgid='PROC00',msg='SERVER: remove pid {}'.format(pid))
                        print('Server remove pid {}'.format(pid))

        # No hay mas archivos por ahora: espero
        log(module=__name__, server='process', function='main', dlgid='PROC00',msg='SERVER: No hay mas archivos: espero')
